...
@pytest.mark.parametrize(
    "shape", [(4, 8, 256), (8, 16), (3, 13, 127), (4, 1)]
)
def test_softmax(dtype, shape):
    a = torch.randn(shape, dtype=dtype, device=DEVICE)
    result = ark.softmax(a).eval()
    expected = F.softmax(a, dim=-1)
    atol = {torch.float32: 1e-5, torch.float16: 1e-3, torch.bfloat16: 5e-2}[dtype]
...
def test_layernorm(dtype, shape):
    ...
    atol = {torch.float32: 1e-4, torch.float16: 1e-2, torch.bfloat16: 5e-2}[dtype]
    assert torch.allclose(
