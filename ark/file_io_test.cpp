...
ark::unittest::State test_create_remove_dir() {
    ...
    UNITTEST_EQ(ark::is_exist(test_dir), false);
...
ark::unittest::State test_list_get_clear_dir() {
    ...
    auto test_file_1 = test_dir + "/test1.txt";
    std::ofstream ofs(test_file_1);
    ofs << "test";
    ofs.close();

    auto test_file_2 = test_dir + "/test2.txt";
    ...
    auto files = ark::list_dir(test_dir);
    UNITTEST_EQ(files.size(), 2UL);
    std::sort(files.begin(), files.end());
    UNITTEST_EQ(files[0], test_file_1);
    UNITTEST_EQ(files[1], test_file_2);

    UNITTEST_EQ(ark::get_dir(test_file_1), test_dir);
    UNITTEST_EQ(ark::get_dir(test_file_2), test_dir);

    UNITTEST_EQ(ark::clear_dir(test_dir), 0);
