...
ark::unittest::State test_all_reduce_qwen3_route_selection() {
    // Qwen3 decode keeps the existing packet route through ark::Model::all_reduce.
    {
        ark::Model m(0, 2);
        ark::Tensor data = m.tensor({4096}, ark::FP16);
    ...
    // Qwen3 prefill uses the explicit prefill route through all_reduce.
    {
        ark::Model m(0, 2);
    ...
    // The direct prefill API is route-testable and preserves in-place output.
    {
        ark::Model m(0, 2);
        ark::Tensor data = m.tensor({2048 * 4096}, ark::FP16);
    ...
    // Unsupported shapes safely fall back to the legacy ring from all_reduce.
    {
        ark::Model m(0, 2);
    ...
    // Packet-misaligned Qwen3 shapes also fall back from all_reduce.
    {
        ark::Model m(0, 3);
        ark::Tensor data = m.tensor({4096}, ark::FP16);
        ark::Tensor output = m.all_reduce(data, 0, 3);
        UNITTEST_FALSE(has_op_type(m, "AllReducePacketFused"));
        UNITTEST_TRUE(has_op_type(m, "Send"));
    }

    // Direct prefill route rejects unsupported shapes with a clear error.
    {
        ark::Model m(0, 2);
        ark::Tensor data = m.tensor({4097}, ark::FP16);
    ...
    // Rank/model mismatch falls back instead of mixing rank authorities.
    {
        ark::Model m(0, 2);
        ark::Tensor data = m.tensor({2048, 4096}, ark::FP16);
        ark::Tensor output = m.all_reduce(data, 1, 2);
        UNITTEST_FALSE(has_op_named(m, "AllReducePacketFused",
                                    "all_reduce_prefill"));
        UNITTEST_TRUE(has_op_type(m, "Send"));
    }
