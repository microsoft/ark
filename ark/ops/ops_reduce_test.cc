...
template <typename T>
void baseline_reduce_mean_axis0(std::vector<void *> &outputs,
                               const std::vector<ark::Dims> &output_shapes,
                               const std::vector<void *> &inputs,
                               const std::vector<ark::Dims> &input_shapes)
{
    ...
    for (ark::DimType c = 0; c < ish[1]; ++c) {
        for (ark::DimType h = 0; h < ish[2]; ++h) {
            for (ark::DimType w = 0; w < ish[3]; ++w) {
                ...
                out[c * osh[2] * osh[3] + h * osh[3] + w] = T(sum / ish[0]);
            }
        }
    }
}
...
template <typename T>
void baseline_reduce_max_axis0(std::vector<void *> &outputs,
                               const std::vector<ark::Dims> &output_shapes,
                               const std::vector<void *> &inputs,
                               const std::vector<ark::Dims> &input_shapes)
{
    ...
    for (ark::DimType c = 0; c < ish[1]; ++c) {
        for (ark::DimType h = 0; h < ish[2]; ++h) {
            for (ark::DimType w = 0; w < ish[3]; ++w) {
                float red = std::numeric_limits<float>::lowest();
                for (ark::DimType n = 0; n < ish[0]; ++n) {
                    float val = float(input[n * ish[1] * ish[2] * ish[3] +
                                       c * ish[2] * ish[3] + h * ish[3] + w]);
                    red = (val > red) ? val : red;
                }
                out[c * osh[2] * osh[3] + h * osh[3] + w] = T(red);
            }
        }
    }
}
...
ark::unittest::State test_reduce_mean_fp16()
{
    {
        ark::Model m;
        ark::Tensor *t = m.tensor(ark::Dims(7, 2, 4, 8192), ark::FP16);
        ark::Tensor *out = m.reduce_mean(t, /*axis=*/0);

        auto result = ark::op_test("reduce_mean_fp16_axis0", m, {t}, {out},
                                   baseline_reduce_mean_axis0<ark::half_t>);
        ark::op_test_log(result);
    }
    {
        ark::Model m;
        ...
        auto result = ark::op_test("reduce_mean_fp16_axis3", m, {t}, {out},
                                   baseline_reduce_mean_axis3<ark::half_t>);
        ark::op_test_log(result);
    }
    return ark::unittest::SUCCESS;
}

ark::unittest::State test_reduce_max_fp16()
{
    {
        ark::Model m;
        ark::Tensor *t = m.tensor(ark::Dims(7, 2, 4, 8192), ark::FP16);
        ark::Tensor *out = m.reduce_max(t, /*axis=*/0);

        auto result = ark::op_test("reduce_max_fp16_axis0", m, {t}, {out},
                                   baseline_reduce_max_axis0<ark::half_t>);
        ark::op_test_log(result);
    }
    {
        ark::Model m;
        ...
        auto result = ark::op_test("reduce_max_fp16_axis3", m, {t}, {out},
                                   baseline_reduce_max_axis3<ark::half_t>);
        ark::op_test_log(result);
    }
    return ark::unittest::SUCCESS;
}
...
int main()
{
    ...
    UNITTEST(test_reduce_mean_fp16);
    UNITTEST(test_reduce_max_fp16);
    return ark::unittest::SUCCESS;
}
