...
struct LayerNorm
{
    ...
    static DEVICE void run(DataType *out, DataType *in, int tx, int ty, int tz)
    {
        ...
        //         int tid_h = ((tid * NelemPerThread) / FinalDim) %
        //         UnitOutShape::H; int tid_c = ((tid * NelemPerThread) /
        //         FinalDim / UnitOutShape::H) %
        //                     UnitOutShape::C;
        //         int tid_n = (tid * NelemPerThread) / FinalDim /
        //         UnitOutShape::H /
        //                     UnitOutShape::C;
        ...
        //             (tid_n + tn * UnitOutShape::N) * InDims::W * InDims::H *
        //             InDims::C;

        //         // smem->storage[tid] = LayerNormType::template
        //         identity<DataType>();
        //         // calculate the sum of the input along the last dimension.
        //         for (int idx_in_w = tid_w; idx_in_w < InShape::W;
        //              idx_in_w += FinalDim) {
        //             int idx_in = idx_in_base + idx_in_w;
        //             DataType LayerNormd = in[idx_in];
        // #pragma unroll
        //             for (int i = 1; i < NelemPerThread; i++) {
        //                 LayerNormd =
        //                     LayerNormType::LayerNorm(LayerNormd, in[idx_in +
        //                     i]);
        //             }
        //             smem->storage[tid] =
