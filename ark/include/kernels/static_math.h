...
// Helper of div.
template <int Divisor, bool IsPow2>
struct Div {};
...
struct Div<Divisor, false> {
    static DEVICE int compute(int x) { return x / Divisor; }
};

// Fast division by pow2 divisor.
template <int Divisor>
static DEVICE int div(int x) {
...
struct Mod<Divisor, true> {
    static DEVICE int compute(int x) { return x & (Divisor - 1); }
};
