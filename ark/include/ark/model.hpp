...
class Model : public ModelGraph {
   ...
   public:
    ...
    Tensor matmul(Tensor input, Tensor other, Tensor output = NullTensor,
                  ...
                  const std::string &name = "");
    Tensor matmul_gelu(Tensor input, Tensor other, Tensor output = NullTensor,
                       bool trans_input = false, bool trans_other = false,
                       const std::string &name = "");
    Tensor matmul_scale(Tensor input, Tensor other, float scale,
                        Tensor output = NullTensor, bool trans_input = false,
                        bool trans_other = false, const std::string &name = "");
    ...
    Tensor mma(Tensor input, Tensor other, Tensor output = NullTensor,
               bool trans_input = false, bool trans_other = false,
               const std::string &name = "");
    Tensor store(Tensor output, Tensor input, const std::string &name = "");
    ...
    Tensor sigmoid(Tensor input, Tensor output = NullTensor,
                   const std::string &name = "");
    // Layer normalization along the last dimension with affine transform (gamma
    // * normalized + beta).
    Tensor layernorm(Tensor input, Tensor gamma, Tensor beta,
                     Tensor output = NullTensor, const std::string &name = "");
    // Softmax along the last dimension (W).
    ...
    Tensor all_reduce_packet(Tensor input, int rank, int rank_num,
                             Tensor output = NullTensor,
                             const std::string &name = "");
    // Explicit Qwen3 prefill all-reduce route. Supports the Qwen3 prefill
    // shape [2048, 4096] (or its flattened equivalent) for FP16 tensors.
