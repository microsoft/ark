...
ark::unittest::State test_codegen_normal_offset() {
    ...
    ark::Model model(0, 2);
    ark::Tensor tns = model.tensor({16, 128}, ark::FP16);
...
ark::unittest::State test_all_reduce_packet_external_input_is_staged() {
    ark::Model model(0, 2);
    ark::Tensor input = model.placeholder({16, 128}, ark::FP16, {}, {}, {}, -1,
        ...
        if (op->type() == ark::ModelOpT::from_name("Copy")) {
            ...
            if (reads[0]->buffer()->id() == placeholder_id) {
                found_copy_from_placeholder = true;
                   ...
                   ark::ModelOpT::from_name("AllReducePacket")) {
            found_fused = true;
            auto reads = op->read_tensors();
            UNITTEST_TRUE(reads.size() > 0);
...
test_all_reduce_packet_invalid_external_input_does_not_mutate_graph() {
    ark::Model model(0, 2);
    ark::Tensor input = model.placeholder({16, 126}, ark::FP16, {}, {}, {}, -1,
                                          reinterpret_cast<void *>(0x1));
...
ark::unittest::State test_all_reduce_packet_internal_input_is_not_staged() {
    ark::Model model(0, 2);
    ark::Tensor input = model.tensor({16, 128}, ark::FP16);
    model.all_reduce_packet(input, 0, 2);
...
ark::unittest::State test_all_reduce_rsag_external_input_is_staged() {
    ark::Model model(0, 2);
    ark::Tensor input = model.placeholder({16, 128}, ark::FP16, {}, {}, {}, -1,
                                          reinterpret_cast<void *>(0x1));
    model.all_reduce_rsag(input, 0, 2);
    ...
    for (auto &node : model.nodes()) {
        ...
        } else if (op->type() == ark::ModelOpT::from_name("AllReduceRsag")) {
            found_rsag = true;
            auto reads = op->read_tensors();
            UNITTEST_TRUE(reads.size() > 0);
            size_t rsag_input_id = reads[0]->buffer()->id();
            UNITTEST_TRUE(rsag_input_id != placeholder_id);
            UNITTEST_TRUE(copy_output_ids.count(rsag_input_id) > 0);
            auto rsag_info =
                ark::BufferRegistry::get_instance().get(rsag_input_id);
            UNITTEST_FALSE(rsag_info && rsag_info->is_external);
        }
    }
    UNITTEST_TRUE(found_copy_from_placeholder);
    UNITTEST_TRUE(found_rsag);

    return ark::unittest::SUCCESS;
}

ark::unittest::State test_all_reduce_rsag_internal_input_is_not_staged() {
    ark::Model model(0, 2);
    ark::Tensor input = model.tensor({16, 128}, ark::FP16);
    model.all_reduce_rsag(input, 0, 2);
...
int main() {
    ...
    UNITTEST(test_all_reduce_rsag_external_input_is_staged);
    UNITTEST(test_all_reduce_rsag_internal_input_is_not_staged);
