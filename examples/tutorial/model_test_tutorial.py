...
class ARKLinear(ark.Module):
    ...
    def forward(self, inputs):
        ...
        return ark.matmul(inputs, self.weight, transpose_other=True)
    
    def backward(self, grad_output):
        grad_weight = ark.matmul(grad_output, self.saved_inputs, transpose_input=True)
...
def compare_grad(ark_model, torch_model, atol=1e-4, rtol=1e-2):
    ...
    for ((ark_name, ark_param), (torch_name, torch_param)) in zip(ark_params, torch_params):
        if (ark_param.grad is None) ^ (torch_param.grad is None):
            print("Exactly one of the gradients is None")
        else:
            grads_equal = torch.allclose(ark_param.grad, torch_param.grad, atol=atol, rtol=rtol)
            if not grads_equal:
                print(f"Gradient for {ark_name} when compared to {torch_name} is different:")
...
def replace_layers_with_ark(model):
    weight_0 = torch.nn.Parameter(model.layers[0].weight.to("cuda:0").requires_grad_(True))
    weight_1 = torch.nn.Parameter(model.layers[1].weight.to("cuda:0").requires_grad_(True))
