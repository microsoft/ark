...
input = ark.tensor([1, 64, 64], dtype=ark.FP32)

reduce = ark.reduce_mean(input, axis=input.ndims() - 1)

other = ark.tensor([1, 64, 64], dtype=ark.FP32)

output = ark.add(reduce, other)

runtime.launch()

input_numpy = np.ones([1, 64, 64], dtype=np.float32)
other_numpy = np.ones([1, 64, 64], dtype=np.float32)
